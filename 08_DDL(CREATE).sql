...
-- 비밀번호 : MEMBER_PM: 자료형 : VARCHAR2(20)
...
INSERT INTO "MEMBER"
VALUES('MEM03', 'ASDFXCV', '박철수', '970303-1234567', DEFAULT);
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
 --> 다른 테이블이 제공하는 컬럼에는 PK 또는 U 제약 조건이 
 --> 설정되어 있어야 한다!! ( 대부분 PK )
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK1 
  REFERENCES USER_GRADE(GRADE_CODE) -- FK 설정 조건을 컬럼 레벨로 함
);
...
CREATE TABLE USER_USED_FK3(
  ...
  ON DELETE CASCADE -- 부모 키 삭제 시 참조하는 자식의 행도 모두 삭제
);
...
SELECT * FROM USER_GRADE3;
SELECT * FROM USER_USED_FK3; -- GRADE_CODE 가 10인 애들이 모두 사라짐
...
--> NOT NULL 제약 조건을 제외한 다른 제약 조건은 복사 X
...
REFERENCES DEPARTMENT(DEPT_ID); -- DEPARTMENT 테이블 PK 컬럼 참조
