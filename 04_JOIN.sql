...
-- 1) 연결에 사용할 두 컬럼명이 다른 경우 --> JOIN 테이블명 ON 사용
...
-- ANSI 방식
SELECT * FROM DEPARTMENT;-- LOCATTION_ID
...
--         연결되지 않음 --> 최종 결과에서 제외됨 == INNER JOIN
-- INNER JOIN에서 INNER는 생략해도 같은 겨로가가 나옴
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); --NULL 포함
...
--> 모든 경우의 수를 보고싶을 때 빼고는 직접 작성하는 경우가 없음
     ...
     -- ( 교차 조인 결과가 보이면 -> 자연 조인 잘못썼다고 보면 됨)

SELECT EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
CROSS JOIN DEPARTMENT -- 모든 경우의 수가 나옴 207행
...
SELECT EMP_NAME, SALARY, E.SAL_LEVEL, MIN_SAL, SALARY, MAX_SAL
FROM EMPLOYEE E
-- JOIN SAL_GRADE S ON (E.SAL_LEVEL = S.SAL_LEVEL) // 등가 조인
...
-- 오라클 구문
SELECT EMP.EMP_NAME "사원명", MGR.EMP_NAME "사수명"
FROM EMPLOYEE EMP, EMPLOYEE MGR -- 오라클은  FROM 절에 테이블 명을 쓴다
...
-- 둘다 저장하고 있는 값이 같고 자료형도 CHAR(2)로 같다
...
SELECT EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE 
NATURAL JOIN JOB; --오라클이 알아서 분석해서 값과 이름이 같은 칼럼을 조인해줌
-- 코드 쓰기가 편해짐 

/* NATURAL JOIN 실패 -> CROSS JOIN 결과 조회 */
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE 
NATURAL JOIN DEPARTMENT; -- 207행 모든 경우의 수가 나옴
---------------------------------------------------------------------------------------------------------------
...
FROM EMPLOYEE 
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 오류남
...
WHERE EMPLOYEE.JOB_CODE =JOB.JOB_CODE -- 1번 조인
...
---------------------------------------------------------------------------------------------------------------
-- 부서코드 :  DEPT_CODE  사원명: EMP_NAME  사번 EMP_ID 직급명  JOB_NAME
-- [연습문제]
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME, SALARY, SALARY*12 *TO_NUMBER(1||BONUS)연봉
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
...
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, LOCAL_NAME 지역명, DECODE(NATIONAL_CODE IN ('KO', 'JP'), 'KO','한국','JP','일본') 국가명
FROM EMPLOYEE
JOIN  DEPARTMENT ON (DEPT_CODE= DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
