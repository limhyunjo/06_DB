...
-- SYSTIMESTAMP : 현재 시간 ( 년, 월, 일, 시, 분, 초, ms + 지역(llocal) ) 조회
...
-- 현재 시간 - 생년월일
SELECT CEIL( (SYSDATE - TO_DATE('1999-04-12', 'YYYY-MM-DD'))/365 ) 
FROM DUAL;
...
SELECT EMP_ID ||EMP_NAME -- ex) 200선동일
FROM EMPLOYEE;
...
--  컬럼명 = NULL  / 컬럼명 != NULL   (X)
  ...
  --> DB에서 NULL 은 값 X, 저장된 값이 없다 라는 의미!!!
...
FROM EMPLOYEE 
WHERE DEPT_CODE IS NULL; -- 저장된 값이 없는 NULL만 나옴 // 2행
