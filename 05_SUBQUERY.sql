...
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) -- 평균 급여
ORDER BY JOB_CODE;
...
-- 부서 별(부서가 없는 사람 포함) 급여의 합계 중 가장 큰 부서의
-- 부서명, 급여 합계를 조회 
...
HAVING SUM(SALARY) = 
	(SELECT MAX(SUM(SALARY))
	FROM EMPLOYEE
	GROUP BY DEPT_CODE); -- 서브쿼리 (17700000)
...
-------------------------- 연습문제 -------------------------------
-- 1. 노옹철 사원과 같은 부서, 같은 직급인 사원을 조회하시오. 
-- (단, 노옹철 사원은 제외)
