...
GRANT CREATE VIEW TO KH_LHJ; -- 권한 부여하고 실행해야 함
...
-- 원본 테이블에도 INSERT 되었는지 확인
SELECT * FROM DEPT_COPY2;
--> 원본도 들어감 DEPT_TITLE에는 NULL이 들어감 ( 원본 DEPT_TITLE이 NULL 허용이라 가능)
...
--> 시퀀스는 MAXVALUE 값을 초과하여 증가할 수 없다
...
-- 인덱스 사용 방법 ! -> WHERE 절에  INDEX가 지정된 컬럼 언급 하기 

SELECT * FROM EMPLOYEE; -- 인덱스 사용 X

SELECT * FROM EMPLOYEE  -- 인덱스 사용 O
WHERE EMP_ID != 0; --> PRIMARY KEY가 지정된 사번 컬럼 언급만 함
...
SELECT COUNT ( * ) FROM TB_IDX_TEST; --1,000,000


-- 인덱스 사용 X
SELECT * FROM TB_IDX_TEST 
WHERE TEST_ID = 'TEST500000'; --0.021

-- 인덱스 사용 O
SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO = 500000; --0.001

--> SELECT조회할 컬럼이 많은 테이블에서는 INDEX를 사용하자!!!
