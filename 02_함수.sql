...
CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/12)|| '년차' "N년차" -- ||  숫자 뒤에 '년차'를 더함
FROM EMPLOYEE; 


-- MONTHS_BETWEEN()은 윤년( 2월 29일 이 포함된 해 ) 계산이 자동으로 수행
...
-- 다음 달 첫 번째, 마지막 날짜
SELECT LAST_DAY(SYSDATE) + 1,  -- 지난달에서 하루만 더하면 다음 달 1일
...
-- 50 미만 확인
SELECT TO_DATE('49-11-25', 'YY-MM-DD') "YY", --2049
...
/* NULL이랑 산술 연산을 수행 시 결과는 무조건 NULL 이다!!!*/
-- EMPLOYEE 테이블에서
-- 이름, 급여, 성과급(급여*보너스)
-- 단, 성과급이 없으면 0으로 표시
...
-- SELECT 절에 작성된 컬럼이 아니라고 해서
-- ORDER BY 절에 사용 못하는게 아님!!
--> FROM 절에서 해석되서 사용 가능한 상태
...
SELECT COUNT(*)
FROM EMPLOYEE
WHERE PHONE IS NOT NULL; -- 20 행

-- 전화번호가 있는 사원의 수 -V2
--> NULL이 아닌 행의 수만 카운트
SELECT COUNT(PHONE) FROM EMPLOYEE; -- 20 행 (NULL은 미포함됨)

-- EMPLOYEE 테이블에 존재하는 부서코드의 수를 조회
-- (EMPLOYEE 테이블에 부서코드가 몇종류?)
SELECT COUNT( DISTINCT DEPT_CODE )FROM EMPLOYEE;
--7행 나와야 될 것 같은데 6행만 나옴!
--> COUNT( ) 내에 컬럼명을 작성하면 NULL은 제외
...
-- SUBSTR( 컬럼명, 자를 수의 개수, 얼마나 잘라서 반환할 지)
